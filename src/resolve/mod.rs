...
impl Resolver {
    pub fn new(project_root: PathBuf, document_root: PathBuf, tempdir: PathBuf, relative_to: RelativeTo) -> Self {
        Resolver {
    ...
    pub fn resolve(
        ...
        let target = Target::new(url, context, &self.project_root, &self.document_root, &self.relative_to, &self.permissions, range, diagnostics)?;
